...
class VisualFeatureExtractor(nn.Module):
    def __init__(self, out_channels=640, inner_channels=256, dropout=0.6):
        super(VisualFeatureExtractor, self).__init__()
        in_cat = 464
        out_cat = out_channels - in_cat
        cnn_temp = torchvision.models.efficientnet_b7(
            weights=torchvision.models.EfficientNet_B7_Weights.IMAGENET1K_V1).features
        self.layer1 = nn.Sequential(cnn_temp[:4])  # 64
        self.layer2 = nn.Sequential(cnn_temp[4:5])  # 32
        self.layer3 = nn.Sequential(cnn_temp[5:6])  # 16
        self.feature_drop = nn.Dropout2d(p=dropout)
        self.contour_aspp = torchvision.models.segmentation.deeplabv3.ASPP(in_channels=in_cat, out_channels=out_cat,
    ...
    def forward(self, x, out_size=(40, 40)):
        ...
        texture_feature = torch.cat([x1_u, x2_u, x3_u], dim=-3)

        # 轮廓特征
        contour_feature = self.feature_drop(self.contour_aspp(texture_feature))
        # 拼接组合
        feature = torch.cat([texture_feature, contour_feature], dim=-3)
        return feature
...
class MultiGranularityConsistencyForgeryDetectionNet(nn.Module):
    def __init__(self, feature_channels=640, inner_channels=256, out_channel=1, dropout=0.6, topk=40):
        ...
        self.visual_feature_extractor = VisualFeatureExtractor(out_channels=feature_channels, inner_channels=inner_channels,
                                                               dropout=dropout)
