...
class TNT(nn.Module):
	...
	model = TNT(img_size=256, patch_size=patch_size, outer_dim=outer_dim, inner_dim=inner_dim, depth=12,
	            outer_num_heads=outer_num_heads, inner_num_heads=inner_num_heads, qkv_bias=False,
	            inner_stride=inner_stride, **kwargs)
    """
	
	def __init__(self, img_size=256, patch_size=16, in_chans=3, num_classes=1000, outer_dim=make_divisible(640, 10), inner_dim=make_divisible(40, 4),
	...
	def model_init(self):
		pretrained_dict = torch.load('./tnt_b_82.9.pth.tar')
	...
	def forward(self, x, out_size=(40, 40)):
		...
		B, HW, C = outer_tokens.shape
		feature = outer_tokens[:, 1:, :].transpose(2, 1)
		
		feature =  feature.reshape(B, C, self.patch_size, self.patch_size)
		return feature
...
if __name__ == '__main__':
    model = TNT()
    result = torch.randn(size=(1, 3, 256, 256))
    x = model(result)
    print(x.size())
